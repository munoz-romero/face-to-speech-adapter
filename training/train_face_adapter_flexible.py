"""
Script de entrenamiento flexible para Face Adapter.
Soporta múltiples configuraciones experimentales mediante argumentos de línea de comandos:

1. Identity Priority:
   --lambda_nce 2.0 --lambda_gender 0.05 --lambda_age 0.01 --temperature 0.04

2. Variance Regularization:
   --lambda_var 1.0 (y otros por defecto)

3. Pure Contrastive:
   --lambda_gender 0.0 --lambda_age 0.0 --lambda_nce 1.0

Implementa una nueva pérdida de regularización de varianza (loss_var) para evitar el colapso de modos.
"""
...
class HybridContrastiveLoss(nn.Module):
    def __init__(self, temperature=0.07, lambda_nce=1.0, lambda_rkd=0.5, lambda_gender=0.2, lambda_age=0.1, lambda_var=0.0):
        super().__init__()
    ...
    def forward(self, proj_emb, audio_emb, labels, pred_gender=None, target_gender=None, pred_age=None, target_age=None):
        ...
        # Denominador: Suma de exp de todos (menos el mismo si se quiere, pero SupCon suele incluirlo)
        exp_logits = torch.exp(logits)
        log_prob = logits - torch.log(exp_logits.sum(1, keepdim=True))

        # Numerador: Suma de log-probs solo de los positivos (definidos por la máscara)
        # mask.sum(1) es la cantidad de positivos por ancla
        ...
        # 4. Variance Regularization Loss
        loss_var = torch.tensor(0.0, device=proj_emb.device)
        if self.lambda_var > 0:
            # Varianza por dimensión a lo largo del batch
            # Agregamos epsilon pequeño para estabilidad numérica
            std_v = torch.sqrt(proj_emb.var(dim=0) + 1e-4)
            std_a = torch.sqrt(audio_emb.var(dim=0) + 1e-4)
            # Queremos que la varianza visual imite a la del audio (que es rica)
            loss_var = F.mse_loss(std_v, std_a.detach())
...
def main():
    ...
    # Loss Weights Arguments
    parser.add_argument('--lambda_nce', type=float, default=1.0, help='Weight for NCE/SupCon loss')
    parser.add_argument('--lambda_rkd', type=float, default=0.5, help='Weight for RKD loss')
    parser.add_argument('--lambda_gender', type=float, default=0.2, help='Weight for Gender loss')
    parser.add_argument('--lambda_age', type=float, default=0.1, help='Weight for Age loss')
    parser.add_argument('--lambda_var', type=float, default=0.0, help='Weight for Variance Regularization loss')
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for batch_idx, (images, mels, speaker_ids, gender, age) in enumerate(dataloader):
            ...
            loss, loss_dict = criterion(
                ...
                target_gender=target_gender_fixed,
                pred_age=pred_age,
                target_age=age.float()
            )
            ...
            # Logging
            if batch_idx % 10 == 0:
                accelerator.print(f"Epoch [{epoch+1}/{args.epochs}] Batch [{batch_idx}/{len(dataloader)}] Loss: {loss.item():.4f} (NCE: {loss_dict['loss_nce']:.4f}, RKD: {loss_dict['loss_rkd']:.4f}, Gender: {loss_dict['loss_gender']:.4f}, Age: {loss_dict['loss_age']:.4f}, Var: {loss_dict['loss_var']:.4f})")
                accelerator.log({
                    ...
                    "Train/loss_var": loss_dict['loss_var'],
